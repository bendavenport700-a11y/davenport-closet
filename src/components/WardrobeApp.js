...
export default function WardrobeApp() {
  ...
  const createWardrobe = async (data) => {
    ...
    if (Array.isArray(d)) setWardrobes(p => [...p, ...d]);
  ...
  const createItem = async (data) => {
    ...
    const d = await r.json();
    if (Array.isArray(d)) setItems(p => [...p, ...d]);
  ...
  const filteredItems = () => {
    ...
    if (searchQuery) list = list.filter(i =>
      ...
      (i.brand || []).some(b => b.toLowerCase().includes(searchQuery.toLowerCase()))
    );
    return list;
  };
  ...
  const fetchFromUrl = async (url) => {
    ...
    for (const proxy of PROXIES) {
      try {
        ...
        const meta = (p) => doc.querySelector(`meta[property="${p}"]`)?.getAttribute("content") || doc.querySelector(`meta[name="${p}"]`)?.getAttribute("content") || "";
  ...
  const S = {
    input: {padding:"9px 12px",borderRadius:4,border:"1px solid #252530",background:"#08080f",color:"#e8e4dc",fontSize:13,outline:"none",boxSizing:"border-box",width:"100%",fontFamily:"Georgia,serif"},
    label: {fontSize:10,color:"#555",letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:7,display:"block"},
    modal: {position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16},
  };
  ...
  return (
    <div style={{background:"#0a0a0c",minHeight:"100vh",color:"#e8e4dc",fontFamily:"Georgia,'Times New Roman',serif"}}>
          ...
          {page === "closet" && activeWardrobe && (
            <>
              ...
              <button onClick={() => { setNewItem({...BLANK_ITEM, wardrobe_id:activeWardrobe}); setShowAddItem(true); }}
                style={{padding:"7px 16px",borderRadius:4,border:"none",background:"#c9a84c",color:"#0a0a0c",cursor:"pointer",fontSize:10,letterSpacing:"0.12em",textTransform:"uppercase",fontWeight:700}}>
                + Add Piece
              </button>
            </>
          )}
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:1,marginBottom:40,border:"1px solid #181820",borderRadius:6,overflow:"hidden"}}>
            {[
              ...
              <div key={s.label} style={{padding:"22px 24px",background:"#0d0d12",borderRight:i<3?"1px solid #181820":"none"}}>
                ...
                <div style={{fontSize:24,color:s.gold?"#c9a84c":"#e8e4dc",marginBottom:4}}>{s.value}</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",marginBottom:32,border:"1px solid #181820",borderRadius:6,overflow:"hidden"}}>
            {[
              ...
              <div key={s.label} style={{padding:"16px 22px",background:"#0d0d12",borderRight:i<3?"1px solid #181820":"none"}}>
                ...
                <div style={{fontSize:21,color:s.gold?"#c9a84c":"#e8e4dc"}}>{s.value}</div>
              </div>
            ))}
          </div>
          ...
          {!activeWardrobe && (
            <div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
                ...
                <button onClick={openNewCollectionModal}
                  style={{padding:"6px 14px",borderRadius:4,border:"1px dashed #2a2a38",background:"none",color:"#444",cursor:"pointer",fontSize:10,letterSpacing:"0.12em",textTransform:"uppercase"}}>
                  + New Collection
                </button>
              </div>
                ...
                {wardrobes.map(w => {
                  ...
                  return (
                    <div key={w.id}
                      ...
                      onMouseEnter={e => { e.currentTarget.style.borderColor="#252535"; e.currentTarget.style.transform="translateY(-1px)"; }}
                      onMouseLeave={e => { e.currentTarget.style.borderColor="#181820"; e.currentTarget.style.transform="translateY(0)"; }}>
                      ...
                      <div style={{padding:"18px 20px"}}>
                        ...
                        <button onClick={() => { setActiveWardrobe(w.id); setFilterOccasion(null); setSearchQuery(""); }}
                          style={{width:"100%",padding:"8px 0",borderRadius:4,border:`1px solid ${w.accent}22`,background:`${w.accent}08`,color:w.accent,cursor:"pointer",fontSize:10,letterSpacing:"0.14em",textTransform:"uppercase"}}
                          onMouseEnter={e => e.target.style.background=`${w.accent}16`}
                          onMouseLeave={e => e.target.style.background=`${w.accent}08`}>
                          Open Collection →
                        </button>
                      </div>
                    </div>
          ...
          {activeWardrobe && activeWD && (
            <div>
              ...
              <div style={{display:"flex",gap:6,marginBottom:20,flexWrap:"wrap",alignItems:"center"}}>
                <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search pieces..."
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:8}}>
                {filteredItems().map(item => {
                  ...
                  return (
                    <div key={item.id} className="item-card"
                      style={{background:picked?"#131310":"#0d0d12",borderRadius:6,border:`1px solid ${picked?activeWD.accent+"55":"#181820"}`,cursor:outfitMode?"pointer":"default",overflow:"hidden"}}
                      ...
                      <div style={{height:190,background:item.image?`url(${item.image}) center/cover no-repeat`:"#111118",display:"flex",alignItems:"center",justifyContent:"center",fontSize:48,position:"relative"}}>
                        ...
                        {!outfitMode && (
                          <div className="item-actions" style={{position:"absolute",top:8,right:8,display:"flex",gap:4,opacity:0,transition:"opacity 0.15s"}}>
                            <button onClick={e => { e.stopPropagation(); startEdit(item); }} style={{background:"#000000cc",border:"none",color:"#e8e4dc",borderRadius:3,padding:"5px 8px",cursor:"pointer",fontSize:11}}>✏</button>
                            <button onClick={e => { e.stopPropagation(); deleteItemById(item.id); }} style={{background:"#000000cc",border:"none",color:"#c87878",borderRadius:3,padding:"5px 8px",cursor:"pointer",fontSize:11}}>✕</button>
                          </div>
                        )}
                      ...
                      <div style={{padding:"12px 14px"}}>
                        ...
                        <div style={{display:"flex",gap:3,flexWrap:"wrap",marginBottom:6}}>
                          {(item.occasion||[]).slice(0,2).map(o => <span key={o} style={{background:"#151520",color:"#3a3a3a",borderRadius:2,padding:"2px 6px",fontSize:9}}>{o}</span>)}
                          {(item.season||[]).slice(0,1).map(s => <span key={s} style={{background:"#151520",color:"#3a3a3a",borderRadius:2,padding:"2px 6px",fontSize:9}}>{s}</span>)}
                        </div>
                        {item.url && (
                ...
                <div onClick={() => { setNewItem({...BLANK_ITEM, wardrobe_id:activeWardrobe}); setShowAddItem(true); }}
                  style={{background:"#0a0a0e",borderRadius:6,border:"1px dashed #1e1e28",minHeight:290,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",cursor:"pointer",gap:8}}
      ...
      {/* COLLECTION MODAL — FIXED */}
      {showCollectionModal && (
        <div style={S.modal} onClick={e => e.target===e.currentTarget && setShowCollectionModal(false)}>
            ...
            <div style={{padding:"22px 24px"}}>
              <div style={{marginBottom:16}}>
                ...
                <input autoFocus type="text" value={newCollection.name}
                  ...
                  onChange={e => { setNewCollection(p => ({...p, name:e.target.value})); setCollectionError(""); }}
              ...
              <div style={{marginBottom:16}}>
                ...
                <input type="number" value={newCollection.budget} placeholder="0"
                  onChange={e => setNewCollection(p => ({...p, budget:e.target.value}))}
                  style={S.input} />
              </div>
              <div style={{marginBottom:16}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  {ACCENT_COLORS.map(c => (
                    <div key={c} onClick={() => setNewCollection(p => ({...p, accent:c}))}
                      style={{width:28,height:28,borderRadius:"50%",background:c,cursor:"pointer",border:newCollection.accent===c?"3px solid #e8e4dc":"3px solid transparent",boxSizing:"border-box"}} />
                  ))}
                </div>
              </div>
              {collectionError && (
      ...
      {/* DELETE CONFIRM */}
      {confirmDelete && (
        <div style={{...S.modal, zIndex:300}} onClick={e => e.target===e.currentTarget && setConfirmDelete(null)}>
          <div style={{background:"#0d0d12",border:"1px solid #c8787825",borderRadius:8,width:"100%",maxWidth:380,padding:28}}>
      ...
      {/* ADD / EDIT ITEM */}
      {showAddItem && (
        <div style={S.modal} onClick={e => e.target===e.currentTarget && resetForm()}>
          <div style={{background:"#0d0d12",border:"1px solid #181820",borderRadius:8,width:"100%",maxWidth:600,maxHeight:"94vh",overflowY:"auto"}}>
